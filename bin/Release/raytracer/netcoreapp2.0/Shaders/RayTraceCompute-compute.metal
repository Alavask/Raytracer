...
bool RayTracer_RayTracingApplication_Refract( float3 v,  float3 n,  float niOverNt, thread float3 &refracted)
{
...
float RayTracer_RayTracingApplication_Schlick( float cosine,  float refIndex)
{
    ...
    r0 = r0 * r0;
    return r0 + (1 - r0) * pow(1 - cosine, (float)5);
}
...
{
    float u = (dtid[0] + RayTracer_RandUtil_RandomFloat(randState)) / 1280;
    float v = (dtid[1] + RayTracer_RandUtil_RandomFloat(randState)) / 720;
